...
func TestEnqueueRefUpdateIsIdempotentAndQueuesOneImmutableJob(t *testing.T) {
	...
	first, err := svc.Enqueue(t.Context(), req)
	if err != nil {
		t.Fatal(err)
	}
	second, err := svc.Enqueue(t.Context(), req)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEnqueueRejectsMismatchedRevisionBeforeQueueWrite(t *testing.T) {
	...
	_, err := svc.Enqueue(t.Context(), api.EnqueueRequest{Context: api.Context{TenantID: "tenant-a", RepositoryID: "repo-a", ActorID: "actor-a", RequestID: "request-a"}, Ref: "refs/heads/main", CommitSHA: "wrong", SourceEventID: "event-1", Trigger: api.TriggerRefUpdated})
...
func TestCancelQueuedJobIsTenantScopedAndNeverLaunches(t *testing.T) {
	...
	job, err := svc.Enqueue(t.Context(), api.EnqueueRequest{Context: api.Context{TenantID: "tenant-a", RepositoryID: "repo-a", ActorID: "actor-a", RequestID: "request-a"}, Ref: "refs/heads/main", CommitSHA: "sha-a", SourceEventID: "event-1", Trigger: api.TriggerRefUpdated})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := svc.Cancel(t.Context(), api.Context{TenantID: "tenant-b", RepositoryID: "repo-a", ActorID: "actor-b", RequestID: "request-b"}, job.ID); !errors.Is(err, api.ErrDenied) {
		t.Fatalf("cross tenant cancel = %v", err)
	}
	cancelled, err := svc.Cancel(t.Context(), api.Context{TenantID: "tenant-a", RepositoryID: "repo-a", ActorID: "actor-a", RequestID: "request-a"}, job.ID)
...
func TestRefUpdatedSubscriberQueuesVerifiedEventOnce(t *testing.T) {
	...
	if err := events.Publish(t.Context(), event); err != nil {
		t.Fatal(err)
	}
	if err := events.Publish(t.Context(), event); err != nil {
		t.Fatal(err)

...
func NewSandbox(job api.Job, config Config) (Sandbox, error) {
	...
	return Sandbox{JobID: job.ID, AttemptID: job.AttemptID, TenantID: job.TenantID, RepositoryID: job.RepositoryID, CommitSHA: job.CommitSHA, ConfigurationDigest: job.ConfigurationDigest, RuntimeClass: config.RuntimeClass, Image: config.Image, Command: append([]string(nil), config.Command...), SourceEndpoint: config.SourceEndpoint, SourceCapability: config.SourceCapability, ReadOnlyRootFilesystem: true, DropAllCapabilities: true, DefaultDenyNetwork: true, Volumes: []Volume{{Name: "work", Ephemeral: true}}}, nil
}

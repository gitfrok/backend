...
// to the tenant and query that issued them (SPEC-0035 AC1): a forged or cross-tenant token fails
// signature verification, an expired one fails its deadline, and either yields no content rather
// than an error that distinguishes causes.
//
// A cursor carries an offset into the authorization-derived result set and nothing else — no
// repository scope and no permission outcome. Every page re-derives the caller's scope at query
// time, so a cursor issued before a revocation never serves the revoked content: paging through
...
// encodeCursor signs one page position under the service's key.
func (s *Service) encodeCursor(tenant, text string, mode, offset int, exp time.Time) string {
	claims := cursorClaims{Tenant: tenant, Text: text, Mode: mode, Offset: offset, Exp: exp}
